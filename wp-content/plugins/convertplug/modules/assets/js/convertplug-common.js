            ...
            jQuery.each(iframes, function( index, iframe ){
                ...
                if( youtube !== -1 ){
                    ...
                    if ( iframe.dataset.autoplay === "1" ){
                        ...
                        iframe.src = yt_src + '&autoplay=' + iframe.dataset.autoplay + '&mute=1';
                    } else {
                        iframe.src = yt_src;
                    }
         ...
         */
         _cp_ifb_toggle:function(){
            jQuery(".cp-info-bar").each(function(index, el) {
                ...
                info_bar.find( ".cp-ifb-toggle-btn" ).on( 'click', function() {
            ...
            info_bar.find( ".ib-close" ).on( 'click', function() {
...
jQuery(window).on('load', function (e) {
   ...
   $.data(this, 'cp_check_empty_span', setTimeout(function() {
    var load_after_post = jQuery('.cp-load-after-post').parent().text().trim();
    if( typeof load_after_post !== 'undefined' ){
        var post_lenght = load_after_post.trim().length;
        if( post_lenght == '0'){
    ...
    if( !jQuery( $this ).parents(".cp-form-container").find(".cp-email").length > 0 ){
        ...
        if ( is_only_conversion > 0 ) {
            var cookieTime = modal.data('conversion-cookie-time'),
                cookie = ConvertPlus._getCookie(style_id);
            if (!cookie) {
                if (cookieTime) {
